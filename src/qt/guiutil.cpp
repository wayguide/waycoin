...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("wayguide"))
        return false;
...
{
    // Convert wayguide:// to wayguide:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("wayguide://"))
    {
        uri.replace(0, 12, "wayguide:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WayGuide.lnk";
}
...
{
    return GetAutostartDir() / "wayguide.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("WayGuide-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  wayguide-qt [" + tr("command-line options") + "]                     " + "\n";
